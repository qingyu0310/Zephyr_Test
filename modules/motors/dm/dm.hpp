...
public:
     ...
     */
    struct Snapshot {
        float position, angle, velocity, torque, tmos, tcoil;
    };
    ...
    uint16_t GetCanId() const { return cfg_.can_id; }       // 控制帧CAN ID
    ...
    //--- 设定目标值 ---
    // CtrlData 只认 rad / rad/s，其他单位自动转成协议值后再赋给 target_rad_ / target_omega_
...
private:
    ...
    struct
    {
        float target_rad_   = 0.0f;         // rad（协议直用）
        float target_ang_   = 0.0f;         // ° = rad × kRad2Deg
        float target_vel_   = 0.0f;         // m/s = omega × wheel_r × 0.5 / gearbox_ratio
        float target_omega_ = 0.0f;         // rad/s（协议直用）
